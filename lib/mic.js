...
function mic({
    ...
    const audioStream = new AudioStream({
        debug,
        silenceThreshold: parseInt(exitOnSilence, 10),
        ...
        return new Promise((resolve, reject) => {
            try {
                if (isWindows) {
                    ...
                    const formatEncoding =
                        encoding === 'unsigned-integer' ? 'U' : 'S';
    ...
    function kill() {
        if (!audioProcess) {
            if (debug) console.log('Microphone was alreday killed');
            return Promise.resolve('Microphone was already killed');
        }
        audioProcess.kill('SIGKILL');
        audioProcess = null;
        if (debug) console.log('Microphone process killed');
        return Promise.resolve('Microphone process killed');
    }
